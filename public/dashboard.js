...
// Show/hide "Switch to Lender" button and lock/unlock lender features
...
function updateLenderFeatures(isVerified) {
    ...
    lenderFeatures.forEach(link => {
        if (isVerified) {
            ...
            link.onclick = (e) => {
                e.preventDefault();
                ui.showNotification('Please verify as a lender to access this feature. Click "Switch to Lender" to get verified.', 'info');
            };
        }
    });
...
function displayRecentActivity(activities) {
    ...
    container.innerHTML = activities.map(activity => {
        ...
        const lenderName = activity.lender?.name || 'Unknown Lender';
        const borrowerName = activity.borrower?.name || 'Unknown Borrower';
...
function displayMyItems(items) {
    ...
    container.innerHTML = items.map(item => `
        <div class="bg-white shadow-sm hover:shadow-xl transition-all duration-300 overflow-hidden group border border-gray-100">
            ...
            <div class="relative w-full pb-[100%] overflow-hidden bg-gray-100">
            ...
            <div class="p-5">
                ...
                <!-- Action Buttons - Compact Square Modern Style -->
                <div class="grid grid-cols-2 gap-2">
                    <button onclick="editItem(${item.id})" 
                        class="px-3 py-2.5 bg-black text-white hover:bg-gray-800 transition-all duration-200 flex items-center justify-center gap-2 font-semibold text-xs uppercase tracking-wide">
                        <i data-lucide="edit-3" class="w-3.5 h-3.5"></i>
                        Edit
                    </button>
                    <button onclick="deleteItem(${item.id})" 
                        class="px-3 py-2.5 bg-white text-black border-2 border-black hover:bg-black hover:text-white transition-all duration-200 flex items-center justify-center gap-2 font-semibold text-xs uppercase tracking-wide">
            ...
            <div class="bg-white p-6 rounded-lg shadow">
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4 p-4 bg-gray-50 rounded-lg">
                    ...
                    <div>
                        ...
                        <p class="font-medium text-primary">$${rental.totalPrice}</p>
                    </div>
                    <div>
